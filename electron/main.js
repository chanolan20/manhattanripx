...
function startBackend() {
  ...
  try {
    // ── sql-wasm.wasm path ─────────────────────────────────────────────────────
    // sql.js needs to find sql-wasm.wasm. We set WASM_PATH env so storage.ts
    // can locate it even before __dirname is known inside the bundle.
    const wasmPath = IS_DEV
      ? path.join(__dirname, '..', 'dtf-rip', 'dist', 'sql-wasm.wasm')
      : path.join(process.resourcesPath, 'server', 'sql-wasm.wasm');
...
function setupIPC() {
  ...
  // ── Printer detection (real WMIC on Windows, lpstat on macOS) ─────────────
  ipcMain.handle('printers:detect', async () => {
    try {
      const data = await fetchJSON(`http://localhost:${SERVER_PORT}/api/printers/detect`);
  ...
  // ── Driver installation (pnputil on Windows) ───────────────────────────
  ipcMain.handle('drivers:install', async (_event, infPath) => {
    try {
      const payload = JSON.stringify({ infPath: infPath || null });
      return await new Promise((resolve) => {
        const req = http.request({
          hostname: 'localhost', port: SERVER_PORT,
          path: '/api/drivers/install', method: 'POST',
          ...
          res.on('end', () => {
            ...
            catch { resolve({ success: false, message: 'Parse error', requiresReboot: false }); }
          });
        });
        req.on('error', (e) => resolve({ success: false, message: e.message, requiresReboot: false }));
        req.write(payload);
        req.end();
      });
    } catch (err) {
      return { success: false, message: err.message, requiresReboot: false };
    }
  });
}
